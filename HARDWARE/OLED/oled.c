...
GPIO_InitTypeDef  GPIO_InitStructure;				//定义一个GPIO_InitTypeDef类型的GPIO初始化结构体
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_1;				    //选择要初始化的GPIOB引脚(PB5,PB6,PB7,PB8)
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;	//设置引脚工作模式为通用推挽输出 		
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	//设置引脚输出最大速率为50MHz
	GPIO_Init(GPIOB, &GPIO_InitStructure);			//调用库函数中的GPIO初始化函数，初始化GPIOB中的PB5,PB6,PB7,PB8引脚
...
void Inter_ShowTemper(u8 *p)
{
	OLED_ShowString(10,2,p,16);
	OLED_ShowCHinese(45,2,10);//度
	OLED_ShowString(53,2,"C",16);
}

// 依据WIFI状态显示图片
void Inter_ShowWifiSignal(int nSignal)
{
	
}

void Inter_ShowChargePower()
{
	int i = 0;
	while(1)
	{
...
void Inter_ShowPower(int nStatus)
{
	OLED_DrawBMP(48,0,64,1,Power[nStatus]);
}

void Inter_ShowWeight()
{
	char weightBuf[10] = {"\0"};
	Get_Weight();

	sprintf(weightBuf,"%dg",Weight_Shiwu);
	OLED_ShowString(0,0,weightBuf,10);
	//printf("净重量 = %d g\r\n",Weight_Shiwu); //打印 
}
